...
fcn_set_grid_size(500) # grid width/ height in meters, and generate grid in local environment
...
# Covariate extraction: polygons

The syntax for covariate extraction for polygons is written to resemble that used in the linear referencing approach to enhance code reusability. Of course, covariates extracted as polygons do not have columns of "FMEAS" and "TMEAS" specific to linearly-referenced transects. The structure is otherwise the same.

We begin by extracting all tables as SF.
```{r}
master <- fcn_all_tables_sf()
```

```{r}
df <- fcn_polygon_extract_raster(fishnet, master$strip_transect[1:10,])
